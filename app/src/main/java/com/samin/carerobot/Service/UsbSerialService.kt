...
class UsbSerialService : Service() {
    ...
    val broadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            if (SerialService.INTENT_ACTION_GRANT_USB.equals(intent?.action)) {
                val granted: Boolean =
                    intent?.getExtras()!!.getBoolean(UsbManager.EXTRA_PERMISSION_GRANTED)
                if (granted) {
                    findUSBSerialDevice(true)
                    val grantedIntent = Intent(SerialService.ACTION_USB_PERMISSION_GRANTED)
                    context?.sendBroadcast(grantedIntent)
                } else {
                    val grantedIntent = Intent(SerialService.ACTION_USB_PERMISSION_NOT_GRANTED)
                    context?.sendBroadcast(grantedIntent)
                }
                ...
                val detachedIntent = Intent(SerialService.ACTION_USB_DEVICE_DETACHED)
...
//        Log.d(serviceTAG, "SerialService : onDestroy")
    ...
    private fun setFilter() {
        ...
        filter.addAction(SerialService.INTENT_ACTION_GRANT_USB)
        filter.addAction(UsbManager.ACTION_USB_DEVICE_ATTACHED)
        filter.addAction(SerialService.ACTION_USB_DEVICE_DETACHED)
        ...
        if (usbDrivers!!.count() > 0) {
            ...
            if (!hasPermission) {
                val intent: PendingIntent =
                    PendingIntent.getBroadcast(this, 0, Intent(SerialService.INTENT_ACTION_GRANT_USB), 0)
...
//                            tmpdata.sliceArray(tmpdata.lastIndex-grabageDataSize..tmpdata.lastIndex)
    ...
    inner class IncomingHandler(
        ...
        override fun handleMessage(msg: Message) {
            when (msg.what) {
                SerialService.MSG_BIND_CLIENT -> {
                    clients.add(msg.replyTo)
                    findUSBSerialDevice()
                }
                SerialService.MSG_UNBIND_CLIENT -> clients.remove(msg.replyTo)
